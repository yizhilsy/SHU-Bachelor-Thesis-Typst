...
因此，一个合理的评估多模态表征之间的相关性必须保证其对于高维空间的适应性及突出的非线形相关性捕获能力。在本章节的剩余部分将首先从理论层面简介一些评估表征相关性指标的原理，之后在人造数据表征、神经网络表征、单一模态（视觉）表征、多模态（图文）表征角度上进行多种衡量表征相关性指标的充分对比实验，最终得到一个最佳的指标。该衡量表征相关性的指标将作为第四章中设计评估多模态大型语言模型模态对齐程度算法的基础。
...
== $\I_d\C\o\r$指标的性能与其他指标的性能实验对比
本小节将运用在3.2节中提到的对比实验方法进行实验，来比较多种衡量数据表征相关性的指标的性能。本小节设置了四种实验条件，分别是：人造数据表征、神经网络表征、单一模态（视觉）表征、多模态（图文）表征，以此来全面地衡量多种指标之间的性能差异，有助于我们筛选出最 state of the arts的指标。

=== 在人造的数据表征上评估
在这一小节中，将在人为生成的可控的数据表征上进行多种指标的性能对比实验。人为可控的含义即在构造数据表征的每个维度时，均可以硬性地控制每个维度采样的数据所服从的概率分布（例如正态分布）以及不同数据表征的特定维度之间的数学表达式关系（例如$X$表征集合的第一维与$Y$表征集合的第一维的线形关系$X_(\d\i\m_1) = Y_(\d\i\m_1)$）。因此这些人为生成的数据表征之间的相关性在数学上是直观且可证明的，存在一个客观的真实值。这个真实值可以帮助我们判断各种指标计算出来结果的正确与否。
...
容易推理得到，线形关系的两个数据表征集合$X_1，Y_1$之间的相关性为1，因为$Y_1$完全依赖于$X_1$；螺旋关系的两个数据表征集合$X_2，Y_2$之间的相关性为1，因为可以推理得到@eqt:1dspiral 的结果，表征集合$X_2，Y_2$之间存在一种非线形关系，同时$x_2，y_2$变量均依赖于$theta$，从数学角度上$X_2，Y_2$表征集合的每个数据点都可以用相同的$theta$表示，因此相关性程度为1；而随机关系的两个数据表征集合$X_3，Y_3$之间的相关性为0，因为两个数据表征集合均服从独立的正态分布，因此它们之间没有任何相关性。

$
  x_2^2 + y_2^2 = (theta^2 cos^2 theta + theta^2 sin^2 theta) / (36pi^2) = theta^2 / (36pi^2)
$<1dspiral>

明晰了这三类条件下客观的相关性程度后，应用了多种指标来计算两个一维数据表征集合之间的相关性。具体来说，本文选取了以下指标进行对比实验：线形相关性系数（$"R"^2$），距离相关系数（dCor），$\I_d\C\o\r$指标。本文针对每类条件下的两个一维数据表征集合 $X，Y$，均进行了10次的采样，每次采样都应用这三种指标进行评估，将这10次采样得到的指标结果进行平均后，数据如@tbl:1dcorrresult 所示，其中$I_d plus.circle$代表了两个数据表征集合按维度拼接后得到的表征集合的本征维度。置信指数$p$统计了10次采样的得到的最小值及最大值，以区间的形式展示。
...
确定了这三类条件下的客观的相关性程度后，应用了多种指标来计算两个四维数据表征集合之间的相关性。具体来说，本文选取了典型相关性分析（CCA），距离相关系数（dCor），$\I_d\C\o\r$指标。本文针对每类条件下的两个四维数据表征集合 $X，Y$ 均进行了5000次的随机采样，每次采样都应用这三种指标进行评估，将这5000次采样得到的指标结果进行平均后，数据如@tbl:4dcorrresult 所示，其中$I_d plus.circle$代表了两个数据表征集合按维度拼接后得到的表征集合的本征维度。置信指数$p$统计了5000次采样的得到的最小值及最大值，以区间的形式展示。
...
从@tbl:4dcorrresult 中可以观察到，在随机关系的条件下，这三种指标均可以正确地反映出两个数据表征集合之间的相关性程度，且计算出来的相关性系数均接近0，每次采样得到的$\I_d\C\o\r$指标的置信指数$p$也表现出了很大的波动性，且大部分呈现为较高的值，成功地反映出两个数据表征集合之间的相关性程度非常低；而在部分相关和完全相关的条件下，线形相关性系数（CCA）和距离相关系数（dCor）计算出来的相关性结果都非常低，均无法正确地反映出两个数据表征集合之间的非线形相关性关系，但是$\I_d\C\o\r$指标则可以正确地反映出两个数据表征集合之间的相关性程度，在5000次采样的结果中，在部分相关的条件下，$\I_d\C\o\r$指标的相关性系数的均值为0.35，在完全相关的条件下，$\I_d\C\o\r$指标的相关性系数的均值为0.67，并且这两种条件下的置信指数$p$也非常小，表明两个数据表征集合之间存在明确的相关性。
...
在这一小节中，在原始输入数据的表征和经过MLP神经网络产生的表征上，应用了多种衡量表征相关性的指标去计算这两个表征之间的相关性，以此来评估它们之间的性能。

==== 实验设置
本文的原始输入数据选取了MNIST @mnist 手写数字体图片数据集；神经网络选取了具有15层全连接的隐藏层，每个隐藏层具有784（对应手写数字体图片 $28 times 28$）个神经元，输出层具有10个神经元（对应 $0-9$ 的数字）的MLP神经网络。
...
本实验利用了Leaky ReLU激活函数的特性，通过控制$alpha$的值来控制神经网络对数据表征作用的变换的非线形程度。具体来说，本实验对 $[1.00-0.00]$ 这个区间以步长-0.02均匀地选取$alpha$
的值，并让神经网络对输入数据MNIST手写数字体执行推理。并抽取输入数据的原始表征和神经网络最后一个隐藏层的输出表征，对这两个表征集合进行相关性评估。在本实验中，选取了距离相关性系数（dCor），中心化核对齐 （CKA），奇异值分解典型相关性分析（SVCCA），$\I_d\C\o\r$指标四种指标进行对比实验。
...
在本小节的实验中，再次通过对比实验印证了$\I_d\C\o\r$指标在捕获复杂非线形相关性方面的强大性能。
...
=== 在单一模态（视觉）表征上进行评估
本小节的实验从更实际、真实的角度出发，衡量不同视觉模型在图像分类数据集ImageNet@imagenet 上前向传播（推理）得到的表征之间的相关性。在同一个数据集上，应用不同的视觉模型对其进行推理时产生的表征实际上是对原始的数据表征做了不同的非线形变换得到的结果，因此视觉模型产生的表征之间是存在客观的相关性的。一个合理的衡量表征之间相关性的指标应该能够有效地捕获这种相关性；同时实际的深度学习神经网络模型所产生的表征都是高维的，这也给衡量表征之间相关性的评估指标带来了挑战，能够克服高维带来的负面影响是衡量指标性能的一个关键因素。在本小节中进行对应的对比实验。


==== 实验设置
具体而言，本实验的原始数据集选取的是ImageNet数据集。ImageNet是一个大规模图像识别及分类数据集，广泛用于计算机视觉和深度学习研究。本实验使用的是一个子集ImageNet-1k，包含1000个图像分类类别和超过120万张训练图像。ImageNet数据集如@img:imagenetshow 所示。
...
=== 在多模态表征（图文）上进行评估
在3.6.3小节中，本文已经证明了$I_d\C\o\r$指标在评估单一模态（视觉）表征时的优秀表现。为了充分证明$I_d\C\o\r$指标可以有效地应用于评估多模态大型语言模型模态融合程度的算法，需要确保$I_d\C\o\r$指标在多模态领域中也能有效地衡量不同模态模型在不同维度的潜在空间中产生的表征之间的相关性，本小节将对比不同指标在多模态表征上进行评估的性能。
